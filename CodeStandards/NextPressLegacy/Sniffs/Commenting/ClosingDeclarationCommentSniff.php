...
namespace NextPress\Sniffs\Commenting;
...
class ClosingDeclarationCommentSniff implements Sniff {
    ...
    public function process(File $phpcsFile, $stackPtr) {
        ...
        if ($tokens[$stackPtr]['code'] === T_FUNCTION) {
            ...
            $classNameReal = $tokens[$className]['content'];
            $comment       = '// end class ' . $classNameReal . ';';
        } else {
        ...
        $data = array($comment);
        if (isset($tokens[($closingBracket + 2)]) === false || $tokens[($closingBracket + 2)]['code'] !== T_COMMENT) {
            $next = $phpcsFile->findNext(T_WHITESPACE, ($closingBracket + 2), null, true);
            if (rtrim($tokens[$next]['content']) === $comment) {
                ...
                if ($fix === true) {
                    ...
                    for ($i = ($closingBracket + 2); $i < $next; $i++) {
                ...
                if ($fix === true) {
                    $phpcsFile->fixer->replaceToken($closingBracket, '}' . str_repeat(' ', $this->spaceBefore) . $comment . $phpcsFile->eolChar);
                }
            }
        ...
        if (rtrim($tokens[($closingBracket + 2)]['content']) !== $comment) {
            $fix = $phpcsFile->addFixableError('Expected %s', $closingBracket, 'Incorrect', $data);
            if ($fix === true) {
                $phpcsFile->fixer->replaceToken(($closingBracket + 2), str_repeat(' ', $this->spaceBefore) . $comment . $phpcsFile->eolChar);
            }
...
} // end class ClosingDeclarationCommentSniff;
