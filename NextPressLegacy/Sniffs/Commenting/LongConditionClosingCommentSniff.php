...
class LongConditionClosingCommentSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (($comment === false) || ($tokens[$comment]['line'] !== $endBrace['line'])) {
            if ($lineDifference >= $this->lineLimit) {
                ...
                if ($fix === true) {
                    ...
                    $phpcsFile->fixer->addContent($stackPtr, str_repeat(' ', $this->spaceBefore) . $expected);
                }
            }
        ...
        if (($comment - $stackPtr) !== 2) {
            $error = 'Space not found before closing comment; expected "%s"';
            $data  = [str_repeat(' ', $this->spaceBefore) . $expected];
